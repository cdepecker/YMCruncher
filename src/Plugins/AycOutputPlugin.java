...
public class AycOutputPlugin extends OutputPlugin {
	...
	private static String AYC_COMMENTS = "Converted using YMCruncher";
...
//					CHANNEL_C + CHANNEL_B + CHANNEL_A
	 ...
	 */
	private Vector doAYCCrunch(Vector arrRawData, int intBuffSize)
	{
		...
		// First Byte
		byte bytData = ((Byte)arrRawData.elementAt(0)).byteValue();
		...
		while(curr_index<arrRawData.size())
		{
			...
			else
			{
				...
				// New character
				bytData = ((Byte)arrRawData.elementAt(curr_index)).byteValue();
				arrCrunchedData.add(new Byte(bytData));
	...
	public ArrayList<Byte> doCrunch(String strDestFile, Chiptune chiptune) {
		...
		// The Following array contains 14 or 16 Vector of Bytes (one for each CPC PSG register)
		Vector<Byte> arrRegs[] = new Vector[nbRegs];
		for (int curr_register=0;curr_register < arrRegs.length; curr_register++)
			arrRegs[curr_register] = /*(blnConvertFrequency==true)?
					(Vector)chiptune.getConvertedVectorRegister(curr_register, YMC_Tools.YM_CPC_FREQUENCY):*/
					(Vector)chiptune.getVectorRegister(curr_register);
...
//						}												
		...
		Vector<Vector<Byte>> arrCrunchedRegister = new Vector<Vector<Byte>>();
		...
		for (int curr_register=0;curr_register < arrRegs.length; curr_register++)
		{
			...
			if (BUFF_SIZE_0x100.equals(strBuffSize)){
				...
				arrCrunchedData = doAYCCrunch(arrRegs[curr_register], 0x100);
				Vector arrCrunchedData400 = doAYCCrunch(arrRegs[curr_register], 0x400);
		...
		for (int i=1;i<arrCrunchedRegister.size();i++)
		{
			...
			addLEvar(arrYmBytes, (Byte)(arrBuffLen.elementAt(i)), 1);
			addLEvar(arrYmBytes, offset, 2);
			buffSize += arrBuffLen.elementAt(i);
		...
		{
			Vector arrData = (Vector)(arrCrunchedRegister.elementAt(i));
		...
		if (getBooleanOption(PARAM_EXTENDED_HEADER))
		{
			...
			{				
				...
				/*Vector<Byte> vect15khz = new Vector<Byte>();
				for (int k=0;k<sample15khz.length;k++) vect15khz.add(new Byte(sample15khz[k])); 
				Vector arrCrunchedDrum100 = doAYCCrunch(vect15khz, 0x100);
				Vector arrCrunchedDrum400 = doAYCCrunch(vect15khz, 0x400);*/
