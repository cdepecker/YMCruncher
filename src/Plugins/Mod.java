...
//    int               tempo;
    ...
    void readSequence(Vector<Byte> arrRawChiptune, int offset) throws IOException
    {
        positions = new byte[128];
        song_length_patterns = YMC_Tools.getLEByte(arrRawChiptune, offset);offset++;
        song_repeat_patterns = YMC_Tools.getLEByte(arrRawChiptune, offset);offset++;
    ...
    void readPatterns(Vector<Byte> arrRawChiptune, int offset) throws IOException
    {
        ...
        for (int i=0; i<numpatterns; i++)
        {
            ...
            // Read fully
            for(int j=0;j<patternsize;j++) patterns[i][j] = YMC_Tools.getLEByte(arrRawChiptune, offset++).byteValue();
        }
    }

    static ModInstrument readInstrument(Vector<Byte> arrRawChiptune, int offset) throws IOException
    {
        ModInstrument inst = new ModInstrument();
        inst.name = YMC_Tools.getNTString(arrRawChiptune, offset, false); offset += 22;
        inst.sample_length = YMC_Tools.getBEShort(arrRawChiptune, offset) << 1;offset += 2;
        inst.samples = new byte[inst.sample_length + 8];  // a little padding for interp.
        inst.finetune_value = (int)((byte)((YMC_Tools.getLEByte(arrRawChiptune, offset) & 0x0F) << 4));offset++;
        inst.volume = YMC_Tools.getLEByte(arrRawChiptune, offset) & 0x7F; offset++;
        inst.repeat_point = YMC_Tools.getBEShort(arrRawChiptune, offset) << 1; offset+=2;
        inst.repeat_length = YMC_Tools.getBEShort(arrRawChiptune, offset) << 1; offset+=2;
    ...
    static void readSampleData(Vector<Byte> arrRawChiptune, int offset, ModInstrument inst) throws IOException
    {
        // Read fully
        for(int j=0;j<inst.sample_length;j++) inst.samples[j] = YMC_Tools.getLEByte(arrRawChiptune, offset++).byteValue();
    ...
    Mod(Vector<Byte> arrRawChiptune) throws IOException
    {
...
//        System.out.println(name);
        ...
        // read sequence
        readSequence(arrRawChiptune, offset); offset += 1 + 1 + 128;
        // skip pattern header
        offset+=4;
        // read mod patterns
        readPatterns(arrRawChiptune, offset); offset += numtracks*4*64 * numpatterns;
