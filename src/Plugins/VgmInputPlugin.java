...
public class VgmInputPlugin extends InputPlugin {
	...
	protected Chiptune getPreProcessedChiptune(Vector arrRawChiptune, String strExt) {
		...
		if (strFileType.subSequence(0,3).equals("Vgm"))
		{
			...
			//arrRegistersValues = new Vector<Byte>();
			Vector<Frame> arrFrame = new Vector();
			...
			while (intOffset<arrRawChiptune.size() && !blnEnd)
			{
				...
				if (shtWait>nbSampleFrame){
					...
					byte cmd = (Byte)arrRawChiptune.elementAt(intOffset++);
					switch(cmd)
					{
						case GG_STEREO:
							intOffset++;
						...
						case WAIT_60:
							...
							break;
						case SN76489_WRITE:
							byte bytPSGvalue = (Byte)arrRawChiptune.elementAt(intOffset++);
							int intValue = 0;
						...
						default :
							...
							else YMC_Tools.info("! Unsupported VGM command : 0x" + Integer.toHexString(intCmd) + " [Offset : 0x" + Integer.toHexString(intOffset) + "]");
					}
				}
			}
