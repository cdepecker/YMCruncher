...
/**
 * GUI class that observes the YMCruncher
 * 
...
public class YMC_View implements Observer {
	...
	private static final String strAboutMessage = "YMCruncher's initial purpose was to convert AY-3-891X chiptunes to a packed format such as AYC to allow them to be replayed on the Amstrad CPC machines.\n"
	...
	public YMC_View( final YMC_Model pmodel ) {
		...
		// Menu OutputPlugins
		// Get List of available Output Plugins and add them in the Menu
		final Vector<OutputPlugin> arrOutputPlugins = model.getOutuptPlugins();
		for ( byte b = 0; b < arrOutputPlugins.size(); b++ ) {
			// OutputPlugin
			final OutputPlugin op = arrOutputPlugins.elementAt( b );
