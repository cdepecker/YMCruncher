...
public abstract class OutputPlugin extends Observable {
    ...
    static private List<OutputPlugin> plugins = initPlugins();
    ...
    private static List<OutputPlugin> initPlugins() {
        ...
        List<OutputPlugin> plugins = serviceLoader.stream().map(ServiceLoader.Provider::get).collect(Collectors.toList());
        return plugins;
    }
